...
type commandRunner func(ctx context.Context, stdin string) (string, error)

var cycleLabelPattern = regexp.MustCompile(`^(Cycle\s+\d+)\s*-\s*(.+)$`)

func defaultClaudeRunner(ctx context.Context, stdin string) (string, error) {
	hideCursor()
	defer showCursor()

	outFile, err := os.CreateTemp("", "gralph-claude-output-*.log")
	if err != nil {
