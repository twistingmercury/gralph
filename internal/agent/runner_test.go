...
func TestCommandRunnerStdin(t *testing.T) {
	...
	t.Run("passes exact arguments and stdin without shell interpretation", func(t *testing.T) {
		...
		capture, err := runner.Run(context.Background(), prompt)
		if !assert.NoError(t, err) {
			return
		}
		t.Cleanup(func() { _ = capture.Cleanup() })
...
func TestCommandRunnerErrors(t *testing.T) {
	...
	t.Run("cancellation", func(t *testing.T) {
		...
		go func() {
			...
			resultCh <- runResult{capture: capture, err: runErr}
		}()
...
func TestCommandRunnerOutput(t *testing.T) {
	...
	stdout, err := os.ReadFile(capture.StdoutPath) // #nosec G304 -- path is generated by CommandRunner
	if !assert.NoError(t, err) {
		return
	}
	stderr, err := os.ReadFile(capture.StderrPath) // #nosec G304 -- path is generated by CommandRunner
	if !assert.NoError(t, err) {
		return
	}
	assert.Equal(t, "stdout only\n", string(stdout))
	assert.Equal(t, "stderr only\n", string(stderr))
}

func TestCommandRunnerCleanup(t *testing.T) {
	t.Parallel()
	...
	assert.FileExists(t, capture.StdoutPath)
	assert.FileExists(t, capture.StderrPath)
	assert.NoError(t, capture.Cleanup())
	assert.NoFileExists(t, capture.StdoutPath)
	assert.NoFileExists(t, capture.StderrPath)
	assert.NoError(t, capture.Cleanup(), "cleanup must be idempotent")
}
...
func TestCommandRunnerErrorsHelper(t *testing.T) {
	...
	case "exit-one":
		os.Exit(1)
	case "output":
		if _, err := os.Stdout.WriteString("stdout only\n"); err != nil {
			os.Exit(6)
		}
		if _, err := os.Stderr.WriteString("stderr only\n"); err != nil {
			os.Exit(7)
		}
		os.Exit(0)
	case "block":
