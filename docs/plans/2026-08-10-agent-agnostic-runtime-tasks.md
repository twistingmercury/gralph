...
- [ ] **Task 4 — Implement argument-based prompt transport**
...
- [ ] **Task 5 — Classify invocation outcomes**
  ...
  - Done: missing executable, permission denial, cancellation, and exit status 1 produce distinguishable outcomes.
...
- [ ] **Task 6 — Replace positional loop startup arguments with configuration**
  ...
  - Done: the loop receives its agent command exclusively through configuration; no production path selects a provider implicitly.
...
- [ ] **Task 7 — Make fatal invocation failures non-mutating**
  ...
  - Done: a missing configured executable leaves the PRD byte-for-byte unchanged and returns a wrapped fatal error after one attempted start.
...
- [ ] **Task 8 — Validate input files before loop side effects**
  ...
  - Done: missing, directory, unreadable, and unsupported symlink inputs fail before progress creation and PRD mutation.
...
- [ ] **Task 9 — Add provider-neutral CLI flags**
  ...
  - Done: CLI help documents the generic contract, missing/invalid configuration exits non-zero, and repeated arguments retain order.
...
- [ ] **Task 10 — Remove provider-specific names from core code**
  ...
  - Done: all root-module tests pass and the search returns no provider-specific identifiers in production core code.
...
- [ ] **Task 11 — Remove provider-specific output labels**
  ...
  - Done: no output label assumes a provider-specific response format.
...
- [ ] **Task 12 — Inject loop output writers**
  ...
  - Done: loop tests can run in parallel without mutating `os.Stdout`.
...
- [ ] **Task 13 — Define agent output capture and cleanup**
  ...
  - Done: each output stream and every cleanup path has a focused test.
...
- [ ] **Task 14 — Add a successful fake-agent end-to-end test**
  ...
  - Done: a complete loop is tested through the compiled gralph binary without network access or a real AI provider.
...
- [ ] **Task 15 — Add failure-policy fake-agent end-to-end tests**
  ...
  - Done: the destructive missing-agent behavior from the 2026-08-10 review is prevented by black-box regression tests.
...
- [ ] **Task 16 — Correct e2e timeout classification**
  ...
  - Done: a timed-out process always fails the test instead of satisfying a generic non-zero-exit assertion.
...
- [ ] **Task 17 — Add SIGTERM cancellation**
  ...
  - Done: either signal stops the fake agent, gralph exits non-zero, and the PRD remains unchanged.
...
- [ ] **Task 18 — Terminate agent descendant processes**
  ...
  - Done: cancellation leaves no fake-agent descendant running on every platform for which the behavior is documented as supported.
...
- [ ] **Task 19 — Add optional provider profiles without weakening the generic path**
  ...
  - Done: each profile expands to an ordinary validated `AgentCommand`, and an arbitrary unregistered executable remains fully supported.
...
- [ ] **Task 20 — Replace Claude-specific repository automation**
  ...
  - Done: no required GitHub workflow depends on Anthropic actions or `CLAUDE_CODE_OAUTH_TOKEN`.
...
- [ ] **Task 21 — Update the user and architecture contracts**
  ...
  - Done: the documented default path requires no named provider, and Claude, Codex, and Grok appear only as optional examples or verified profiles.
...
- [ ] **Task 22 — Add the final provider-agnostic acceptance gate**
  ...
  - Test: `make verify`
  - Done: `make verify` passes from a clean checkout, fake-agent tests cover both prompt modes, and `rg -n 'defaultClaudeRunner|invokeClaude|CLAUDE_CODE_OAUTH_TOKEN' cmd internal .github/workflows` returns no matches.
